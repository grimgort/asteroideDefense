                          ...
                          CommandQueue& commands)
{
    if (event.type == sf::Event::KeyPressed)
    {
        ...
        if (m_keyBinding
                && m_keyBinding->checkAction (event.key.code, action)
    ...
    if ( (event.type == sf::Event::KeyPressed
            || event.type == sf::Event::KeyReleased) && m_socket)
    {
        Action action;
...
void Player::handleRealtimeInput (CommandQueue& commands)
{
    if ( (m_socket && isLocal()) || !m_socket)
    {
        std::vector<Action> activeActions =
            m_keyBinding->getRealtimeActions();
...
void Player::initializeActions()
{
    ...
    m_actionBinding[PlayerAction::MoveLeft].action =
        derivedAction<Aircraft> (AircraftMover (-1, 0, m_identifier));
    m_actionBinding[PlayerAction::MoveRight].action =
        derivedAction<Aircraft> (AircraftMover (+1, 0, m_identifier));
    m_actionBinding[PlayerAction::MoveUp].action =
        derivedAction<Aircraft> (AircraftMover (0, -1, m_identifier));
    m_actionBinding[PlayerAction::MoveDown].action =
        derivedAction<Aircraft> (AircraftMover (0, +1, m_identifier));
