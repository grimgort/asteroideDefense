    ...
    m_GUIContainer()
{
    ...
    for (std::size_t x = 0; x < 2; ++x)
    {
        ...
        addButtonLabel (PlayerAction::LaunchMissile, x, 5, "Missile",
                        context);
        addButtonLabel (PlayerAction::LaunchAsteroideUn, x, 6, "AsteroideUn",
                        context);
    }
    ...
    backButton->setCallBack (std::bind (&SettingsState::requestStackPop,
                                        this));
...
void SettingsState::addButtonLabel (std::size_t index, std::size_t x,
                                    std::size_t y, const std::string& text, Context context)
{
    index += PlayerAction::Count * x;

    m_bindingButtons[index] = std::make_shared<GUI::Button> (context);
    m_bindingButtons[index]->setPosition (400.f * x + 80.f,
                                          50.f * y + 150.f);
    ...
    m_bindingLabels[index] = std::make_shared<GUI::Label> ("",
                             *context.fonts);
    m_bindingLabels[index]->setPosition (400.f * x + 300.f,
                                         50.f * y + 150.f);
