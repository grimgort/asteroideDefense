...
MultiplayerGameState::MultiplayerGameState (StateStack& stack,
    ...
    , m_timeSinceLastPacket (sf::seconds (0.f))
{
    ...
    m_playerInvitationText.setPosition (1000 -
                                        m_playerInvitationText.getLocalBounds().width,
                                        760 - m_playerInvitationText.getLocalBounds().height);
    ...
    m_failedConnectionText.setPosition (m_window.getSize().x / 2.f,
    ...
    if (m_socket.connect (ip, ServerPort,
                          sf::seconds (5.f)) == sf::TcpSocket::Done)
    { m_connected = true; }
    else
...
void MultiplayerGameState::draw()
{
    if (m_connected)
    {
        ...
        if (m_localPlayerIdentifiers.size() < 2
                && m_playerInvitationTime < sf::seconds (0.5f))
...
bool MultiplayerGameState::update (sf::Time dt)
{
    if (m_connected)
    {
        ...
        {
            if (std::find (m_localPlayerIdentifiers.begin(),
                           m_localPlayerIdentifiers.end(),
                           itr->first) != m_localPlayerIdentifiers.end())
            {
...
bool MultiplayerGameState::handleEvent (const sf::Event& event)
{
    ...
    if (event.type == sf::Event::KeyPressed)
    {
        if (event.key.code == sf::Keyboard::Return
                && m_localPlayerIdentifiers.size() == 1)
        {
            sf::Packet packet;
...
void MultiplayerGameState::updateBroadcastMessage (
    sf::Time elapsedTime)
{
    if (m_broadcasts.empty())
    { return; }
        ...
        sf::Packet& packet)
{
    ...
    case Server::SpawnSelf:
    {
        ...
        m_players[aircraftIdentifier].reset (new Player (&m_socket,
                                             aircraftIdentifier, getContext().keys1));
 ...
 //       aircraft->setPosition(m_world.getWorldBounds().top) ;
    ...
    case Server::SpawnEnemy:
    {
        ...
        m_world.addEnemy (static_cast<Aircraft::Type> (type), relativeX,
                          height);
        m_world.sortEnemies();
    }
    break;
    ...
    case Server::UpdateClientState:
    {
        ...
        float currentViewPosition = m_world.getViewBounds().top +
                                    m_world.getViewBounds().height;

        m_world.setWorldScrollCompensation (currentViewPosition /
        ...
        for (sf::Int32 i = 0; i < aircraftCount; i++)
        {
            ...
            bool isLocalPlane = std::find (m_localPlayerIdentifiers.begin(),
                                           m_localPlayerIdentifiers.end(),
                                           aircraftIdentifier) != m_localPlayerIdentifiers.end();
