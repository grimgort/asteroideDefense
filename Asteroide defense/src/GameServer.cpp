...
void GameServer::handleIncomingPacket(sf::Packet& packet, RemotePeer& receivingPeer, bool& detectedTimeout)
{
    ...
    case Client::GameEvent:
    {
        ...
        if (action == GameActions::EnemyExplode && randomInt(3) == 0 && &receivingPeer == m_peers[0].get())
        {
            sf::Packet packet;
...
void GameServer::informWorldState(sf::TcpSocket& socket)
{
    ...
    for (std::size_t i = 0; i < m_connectedPlayers; ++i)
    {
        if (m_peers[i]->ready)
        {
            ...
            packet << identifier << m_aircraftInfo[identifier].position.x << m_aircraftInfo[identifier].position.y << m_aircraftInfo[identifier].hitpoints << m_aircraftInfo[identifier].missileAmmo;
        }
    }
