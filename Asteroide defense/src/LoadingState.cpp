...
LoadingState::LoadingState (StateStack& stack, Context context)
    : State (stack, context)
{
    //On applique la texture de chargement
    ...
    m_progressBarBackground.setSize ( (sf::Vector2f (viewSize.x - 20,
                                       10)));
    m_progressBarBackground.setPosition (10,
                                         m_loadingText.getPosition().y + 40);
...
void LoadingState::setCompletion (float percent)
{
    ...
    m_progressBar.setSize (sf::Vector2f (
                               m_progressBarBackground.getSize().x * percent,
