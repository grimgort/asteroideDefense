...
{
public:
    void load(Identifier id, const std::string& filename); //charge une texture depuis son identifiant(Identifier) et son image (filename)
    ...
    const Resource& get(Identifier id) const; //get resource without modification
