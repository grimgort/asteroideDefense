        ...
        const std::string& filename, const Parameter& secondParam)
{
    ...
    */
    ...
    /*resource is load and we verify that return true, else that return error.*/
    if (!resource->loadFromFile (filename,
                                 secondParam)) //utilise la méthode loadFromFile de la classe pointé par resource
    { throw std::runtime_error ("ResourceHolder::load - Failed to load " + filename); }

    /*  resource is inserted with "id" key and
        return boolean value in "inserted". if inserted is false, error is returned with assert (debug mode only) */
    insertResource (id, std::move (
                        resource));  //Permet de stocker le pointeur de la classe resource dans l'attribut m_resourceMap
}
/*
...
{
    auto found = m_resourceMap.find (
                     id);   //cherche dans le tableau (map) l'id et stock la ligne dans "found".
    /*if "id" is not included in m_textureMap error is returned with assert*/
    assert (found !=
            m_resourceMap.end());  //retourne une erreur si pas trouvé.
